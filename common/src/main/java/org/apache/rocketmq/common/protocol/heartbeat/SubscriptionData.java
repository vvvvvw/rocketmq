...
public class SubscriptionData implements Comparable<SubscriptionData> {
    ...
    private boolean classFilterMode = false; //是否是类过滤模式，默认为 false
    private String topic; //消息主题名称
    private String subString; //消息过滤表达式，多个用双竖线隔开，例如"TAGA ||TAGB"
    private Set<String> tagsSet = new HashSet<String>(); //消息过滤 tag 集合 ，消费端过滤时进行消息过滤的依据
    private Set<Integer> codeSet = new HashSet<Integer>(); //消息过滤 tag hashcode集合
    private long subVersion = System.currentTimeMillis();
    private String expressionType = ExpressionType.TAG; //过滤类型， TAG 或 SQL92
